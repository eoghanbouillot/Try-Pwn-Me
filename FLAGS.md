# 🚩 FLAGS - TRY-PWN-ME

## ⚠️ ATTENTION - SPOILERS !

Ce fichier contient tous les flags des challenges. Ne regardez pas si vous voulez résoudre les challenges par vous-même !

---

## 🌐 WEB

### Challenge 01 - User-Agent
**Flag:** `TPM{us3r_ag3nt_m4st3r_2024}`

### Challenge 02 - Source Code
**Flag:** `TPM{s0urc3_c0d3_1s_y0ur_fr13nd}`

### Challenge 08 - SQL Injection
**Flag:** `TPM{sql_1nj3ct10n_byp4ss_succ3ss}`

### Challenge 09 - Cookie Manipulation
**Flag:** `TPM{c00k13s_4r3_d3l1c10us_but_1ns3cur3}`

### Challenge 10 - OSINT Genesis
**Flag:** `TPM{The_Times_03/Jan/2009_Chancellor_on_brink_of_second_bailout_for_banks}`

---

## 🔐 CRYPTOGRAPHIE

### Challenge 03 - Caesar Cipher
**Flag:** `TPM{c4e14r_i4_n0t_4_good_encrrypti0n}`


### Challenge 04 - Base64 Multi-Layers
**Flag:** `TPM{b4s364_1s_n0t_3ncryp7i0n}`

### Challenge 05 - RSA Weak Key
**Flag:** `TPM{w34k_rsa_k3y_1s_br0k3n}`

---

## 💀 PWN / BINARY EXPLOITATION

### Challenge 06 - Buffer Overflow
**Flag:** `TPM{buff3r_0v3rfl0w_m4st3r}`

### Challenge 07 - Format String
**Flag:** `TPM{f0rm4t_str1ng_pwn3d}`

---

## 📊 Résumé

| Challenge | Catégorie | Difficulté | Points | Flag |
|-----------|-----------|------------|--------|------|
| 01 | Web | Facile | 50 | `TPM{us3r_ag3nt_m4st3r_2024}` |
| 02 | Web | Très Facile | 25 | `TPM{s0urc3_c0d3_1s_y0ur_fr13nd}` |
| 03 | Crypto | Facile | 50 | `TPM{c4e14r_i4_n0t_4_good_encrrypti0n}` |
| 04 | Crypto | Moyen | 100 | `TPM{b4s364_1s_n0t_3ncryp7i0n}` |
| 05 | Crypto | Difficile | 200 | `TPM{w34k_rsa_k3y_1s_br0k3n}` |
| 06 | PWN | Moyen | 100 | `TPM{buff3r_0v3rfl0w_m4st3r}` |
| 07 | PWN | Difficile | 200 | `TPM{f0rm4t_str1ng_pwn3d}` |
| 08 | Web | Moyen | 100 | `TPM{sql_1nj3ct10n_byp4ss_succ3ss}` |
| 09 | Web | Facile | 50 | `TPM{c00k13s_4r3_d3l1c10us_but_1ns3cur3}` |
| 10 | OSINT | Difficile | 150 | `TPM{The_Times_03/Jan/2009_Chancellor_on_brink_of_second_bailout_for_banks}` |

**Total : 10 challenges • 1025 points**

---

## 🎓 Leçons de Sécurité

Chaque flag enseigne une leçon importante :

1. **User-Agent** → L'authentification côté client n'est pas sécurisée
2. **Source Code** → Ne jamais stocker de secrets dans le code client
3. **Caesar** → Les algorithmes de chiffrement anciens sont obsolètes
4. **Base64** → L'encodage n'est PAS du chiffrement
5. **RSA** → La taille des clés est cruciale en cryptographie
6. **Buffer Overflow** → Toujours valider les tailles de buffers
7. **Format String** → Ne jamais utiliser printf(user_input)
8. **SQL Injection** → Toujours utiliser des requêtes préparées
9. **Cookies** → Ne jamais faire confiance aux cookies pour les privilèges
10. **OSINT** → Internet n'oublie jamais (Blockchain)

---

*Made with ♥ for hackers by hackers*
